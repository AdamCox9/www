				...
				*/
				...
				if( $buy_price > 0 && ! in_array( $base_cur, $quote_curs ) ) {
					...
					if( $order_size !== FALSE ) {
						...
						echo " -> Buying $order_size of $base_cur in {$market_summary['pair']} on {$market_summary['exchange']} costing $total_buy_cost of $quote_cur at $buy_price with $quote_cur per $base_cur\n\n";
				...
				if( $sell_price > 0 && ! in_array( $base_cur, $quote_curs ) ) {
					...
					if( $order_size !== FALSE ) {
						...
						if( $order_size > 0 ) {
							...
							echo " -> Selling $order_size of $base_cur in {$market_summary['pair']} on {$market_summary['exchange']} gaining $total_sell_gain of $quote_cur at $sell_price with $quote_cur per $base_cur\n\n";
