				...
				*/
				...
				if( $buy_price > 0 && ! in_array( $base_cur, $quote_curs ) ) {
					...
					if( sizeof( $order_size ) > 0 ) {
						...
						echo " -> Buying $order_size of $base_cur in {$market_summary['pair']} on {$market_summary['exchange']} costing $total_buy_cost of $quote_cur at $buy_price $quote_cur per $base_cur\n";
						print_r( $Adapter->buy( $market_summary['pair'], $order_size, $buy_price, 'limit', array( 'market_id' => $market_summary['market_id'] ) ) );
						echo "\n\n";
					}
				}
				...
				if( $sell_price > 0 && ! in_array( $base_cur, $quote_curs ) ) {
					...
					if( sizeof( $order_size ) > 0 ) {
						...
						echo " -> Selling $order_size of $base_cur in {$market_summary['pair']} on {$market_summary['exchange']} gaining $total_sell_gain of $quote_cur at $sell_price $quote_cur per $base_cur\n";
						print_r( $Adapter->sell( $market_summary['pair'], $order_size, $sell_price, 'limit', array( 'market_id' => $market_summary['market_id'] ) ) );
						echo "\n\n";
					}
				}
